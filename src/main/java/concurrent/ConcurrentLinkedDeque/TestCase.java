 ...
 *
 * 然后,你执行100个 RemoveTask任务将元素从列表中删除。每一个任务删除这个列表 用pollFirst()和pollLast()方法。pollFirst()方法返回和删除列表的第一个元素和pollLast()方法返回和删除最后一个元素的列表。
 * 如果列表为空,这些方法返回一个null值。当所有这些任务已经完成了,在控制台的打印出列表中元素的数量，这时,有零元素列表。
 *
 * 打印列表的元素的数量时,你使用了 size()方法，你必须考虑,这种方法并不是真实的,特别是如果你使用它在线程进行列表中添加同时又删除数据。计数的方法遍历整个列表的元素和内容列表可以改变这个操作。
 * 一旦在你使用它们时没有任何线程修改列表,你可以保证返回的结果是正确的。
 *
