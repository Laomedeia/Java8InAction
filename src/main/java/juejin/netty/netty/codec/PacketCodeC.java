...
import juejin.netty.netty.protocol.packet.request.*;
import juejin.netty.netty.protocol.packet.response.*;
...
/**
 * 注册 packetType Map, 可进行(编码/解码)转换二进制的数据包对象
 * @author neptune
...
//    public ByteBuf encode(ByteBufAllocator byteBufAllocator, Packet packet) {
    public ByteBuf encode(ByteBuf byteBuf, Packet packet) {
        // 1. 创建 ByteBuf 对象 (也可以通过ByteBufAllocator.DEFAULT.ioBuffer()创建一个 ByteBuf，这里我们调用 Netty 的 ByteBuf 分配器来创建，ioBuffer() 方法会返回适配 bio 读写相关的内存，它会尽可能创建一个直接内存，直接内存可以理解为不受 jvm 堆管理的内存空间，写到 IO 缓冲区的效果更高)
